...
public class SITE_Area implements Region{

	ResultSet resultSet = null;
	String user, pass;
	
	SITE_Area(String user, String pass){
		this.user = user;
		this.pass = pass;
	}
		...
		String query = "SELECT candidate_first_name, candidate_last_name, cnic, password FROM candidate_registeration WHERE cnic=? AND password=?";
      	...
      	try {
      		...
      		if(resultSet.next()) {
    			...
    			r1.voterOptionScreen(candidate_first_name, candidate_last_name);
	...
	public void loginValidationForVoter() {
    	...
    	try {
    		...
    		if(resultSet.next()) {
    			...
    			r1.candidateOptionScreen(voter_first_name,voter_last_name);
